"""Cega (correlation sensitivity) vs USDJPY spot at different SPX levels."""
import sys, os
...
def main():
    ...
    for S0 in spx_levels:
        cegas = []
        for X0 in fx_grid:
            up = price_at(S0, X0, rho=p.rho + drho)
            dn = price_at(S0, X0, rho=p.rho - drho)
            cegas.append((up - dn) / 2 * units / 1e3)
        ax.plot(fx_grid, cegas, label=f"SPX = {S0}", linewidth=2)
    ...
    ax.set_ylabel("Cega ($k per 1% correlation, on $100mm notional)")
    ax.set_title(f"Cega vs USDJPY Spot, by SPX Level\n"
                 f"(K={p.K:.0f}, B={p.B:.0f}, T={p.T}y, rho={p.rho})\n"
                 f"Bell-shaped, peaks just below barrier")
    ax.legend(loc="lower left", fontsize=9)
    ax.grid(alpha=0.3)
    plt.tight_layout()

    out = os.path.join(os.path.dirname(__file__), "..", "figures", "05_cega.png")
